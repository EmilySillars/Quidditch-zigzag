...
"func.func"() <{function_type =  (memref<2x600xi8>, memref<600x2xi8, strided<[1,600]>>, memref<2x2xi32, strided<[600,1]>>) -> (), sym_name = "dispatch_to_accelerator", sym_visibility = "private"}> ({}) {llvm.emit_c_interface}: () -> ()

// declaring an external MLIR function called hola
"func.func"() <{function_type =  (memref<2x600xi8>, memref<600x2xi8, strided<[1,600]>>, memref<2x2xi32, strided<[600,1]>>) -> (), sym_name = "hola", sym_visibility = "private"}> ({}) {llvm.emit_c_interface}: () -> ()

  // an MLIR func that we would like to have eventually call C code
  "func.func"() <{function_type = (memref<600x600xi8>, memref<600x600xi8, strided<[1, 600]>>, memref<600x600xi32>) -> (), sym_name = "mlirFunc"}> ({
  ^bb0(%arg0: memref<600x600xi8, 0 : i32>, %arg1: memref<600x600xi8, strided<[1,600]>, 0 : i32>, %arg2: memref<600x600xi32>):
    %arg2_diff_stride = memref.cast %arg2 : memref<600x600xi32> to memref<600x600xi32, strided<[600, 1]>>
    func.call @simple_matmul(%arg0, %arg1, %arg2) : (memref<600x600xi8>, memref<600x600xi8, strided<[1, 600]>>, memref<600x600xi32>) -> ()
    //func.call @tiled_matmul(%arg0, %arg1, %arg2_diff_stride) : (memref<600x600xi8>, memref<600x600xi8, strided<[1, 600]>>, memref<600x600xi32, strided<[600, 1]>>) -> ()
 "func.return"() : () -> ()
  }) {llvm.emit_c_interface} : () -> ()
