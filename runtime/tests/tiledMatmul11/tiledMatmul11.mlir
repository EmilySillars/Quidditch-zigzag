...
// "func.func"() <{function_type =  (memref<?x?xi32>, memref<?x?xi32>) -> (), sym_name = "memrefCopy32bit", sym_visibility = "private"}> ({}) {llvm.emit_c_interface}: () -> ()
"func.func"() <{function_type = (memref<104x13xi8, strided<[1, 104], offset: ?>>, memref<104x13xi8, strided<[1, 104], offset: ?>>) -> (), sym_name = "memrefCopy8bit", sym_visibility = "private"}> ({}) {llvm.emit_c_interface}: () -> ()
...
// dispatching part of the work to the accelerator!
  "func.func"() <{function_type = (
    ...
    memref<104x104xi32, strided<[104,1]>>) 
    -> (), sym_name = "tiled_matmul"}> ({
  ^bb0(
    %arg0: memref<104x104xi8>, 
...
// dispatching part of the work to the accelerator!
  ...
  (memref<104x104xi8>,                        // arg0 (input)
   ...
   memref<104x13xi32, strided<[104,1]>>,     // arg3 (output slice l1)
   memref<104x13xi8, strided<[1, 104]>>      // arg4 (weight slide l1)
  ) -> (), sym_name = "pineapple"}> ({
  ^bb0(%arg0: memref<104x104xi8>, 
       %arg1: memref<104x104xi8, strided<[1,104]>>, 
       %arg2: memref<104x104xi32, strided<[104,1]>>, 
       %l1OSlice: memref<104x13xi32, strided<[104,1]>>,
       %l1WSlice: memref<104x13xi8, strided<[1,104]>>):
    ...
    %b1_0_bk_sz = arith.constant 1: index
   
    // constants
    %zero_i32 = arith.constant 0: i32
    %sixTwentyFour = arith.constant 624: i32
    ...
    : memref<104x13xi32, strided<[104,1]>> to memref<104x13xi32, strided<[104, 1], offset: ?>>    

    // slices of Weight @(104x13)
    ...
    %verticalSliceWL1 = memref.subview %l1WSlice[%zero,%zero][104,13][1,1]
    : memref<104x13xi8, strided<[1, 104]>>  to memref<104x13xi8, strided<[1, 104], offset: ?>> 

    // copy Weight L3 to L1
    ...
    %b1 = arith.muli %b1_0, %b1_0_bk_sz : index
    
    // slices of Output @(104x1)
    %vSliceOL3 = memref.subview %verticalSliceOL3[%zero, %b1][104,1][1,1] 
    :  memref<104x13xi32, strided<[104, 1], offset: ?>> to memref<104x1xi32, strided<[104, 1], offset: ?>>
    %vSliceOL1 = memref.subview %verticalSliceOL1[%zero, %zero][104,1][1,1]
    : memref<104x13xi32, strided<[104, 1], offset: ?>> to memref<104x1xi32, strided<[104, 1], offset: ?>>
    
    // slice of Weight @(104x1)
    %vSliceWL1 = memref.subview %verticalSliceWL1[%zero,%zero][104,1][1,1]
   ...
   // FAKE DISPATCH
    ...
    %val = memref.load %vSliceOL1[%i, %zero] : memref<104x1xi32, strided<[104, 1], offset: ?>>
    %newVal = arith.addi %val, %one_i32 : i32
     memref.store %newVal, %vSliceOL1[%i, %zero] : memref<104x1xi32, strided<[104, 1], offset: ?>>
    } // end of %i for
  ...
  ^bb0(
    ...
    scf.for %a1_0 = %zero to %thirteen step %one iter_args () -> () { 
      ...
      %c0 = arith.addi %prod0, %c1_0 : index
