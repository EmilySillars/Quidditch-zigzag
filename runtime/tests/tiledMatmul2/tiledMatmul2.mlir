...
"func.func"() <{function_type = (memref<16x16xi8>, memref<16x16xi8, strided<[1, 16]>>, memref<16x16xi32, strided<[16,1]>>) -> (), sym_name = "tiled_matmul"}> ({
  ^bb0(%arg0: memref<16x16xi8>, %arg1: memref<16x16xi8, strided<[1,16]>>, %arg2: memref<16x16xi32, strided<[16,1]>>):
    // 32 bit integers ?
    %0 = "arith.constant"() <{value = 0 : i32}> : () -> i32
    // %d0_1_bk_sz = "arith.constant"() <{value = 4 : i32}> : () -> i32 //arith.constant 4 : index
    // %d1_1_bk_sz = "arith.constant"() <{value = 8 : i32}> : () -> i32 //arith.constant 8 : index
    // %d1_2_bk_sz = "arith.constant"() <{value = 4 : i32}> : () -> i32 //arith.constant 4 : index
    // %d2_1_bk_sz = "arith.constant"() <{value = 8 : i32}> : () -> i32 //arith.constant 8 : index
