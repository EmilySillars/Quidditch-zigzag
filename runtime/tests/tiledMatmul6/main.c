...
int main() {
  ...
  matmul_transformed(&memrefA, &memrefB, &memrefC);
     ...
     for (size_t d0_2 = 0; d0_2 < 8; d0_2++) {
      for (size_t d1_2 = 0; d1_2 < 8; d1_2++) {
       for (size_t d2_2 = 0; d2_2 < 8; d2_2++) {
        ...
        z->aligned_data[z_index] +=
            x->aligned_data[x_index] * y->aligned_data[y_index];
        //z->mat[d0][d1] += x->mat[d0][d2] * y->mat[d2][d1];
       }
      }
     }
    }
...
void matmul_transformed2(TwoDMemrefI8_t *x, TwoDMemrefI8_t *y,
  ...
  for (size_t d0_1 = 0; d0_1 < 13; d0_1++) { // 13 8-elt-row-long blocks
  //Temporal Loops                   O            W            I      
  //for D0 in [0, 13):               l3           l1           l1
  // output slice in L3
  // move input slice into L1
  // move weight "slice" into L1     
     ...
     for (size_t d0_2 = 0; d0_2 < 8; d0_2++) {
      for (size_t d1_2 = 0; d1_2 < 8; d1_2++) {
       for (size_t d2_2 = 0; d2_2 < 8; d2_2++) {
        ...
        //perform MAC
        z_index = (d0 * MAT_WIDTH) + d1; // O
        x_index = (d0 * MAT_WIDTH) + d2; // I 
        y_index = (d2 * MAT_WIDTH) + d1; // W
        z->aligned_data[z_index] +=
            x->aligned_data[x_index] * y->aligned_data[y_index];
        //z->mat[d0][d1] += x->mat[d0][d2] * y->mat[d2][d1];
       }
