...
static void park_worker() {
  worker_metadata.workers_waiting++; // maintains information about the cluster; uses atomic vars because ANY core can access this
  asm volatile("wfi"); // sleep until you get woken up by cluster local interrupt
  snrt_int_cluster_clr(1 << snrt_cluster_core_idx()); // immediately clear the interrupt because we have woken up
...
static void wake_all_workers() {
  ...
  snrt_int_cluster_set((1 << compute_cores) - 1); // use bitmask to wake up compute cores using a local cluster interrupt!!!
}
...
int quidditch_dispatch_enter_worker_loop() {
  snrt_interrupt_enable(IRQ_M_CLUSTER); // extremely important! enables interrupts on the cluster!!!
